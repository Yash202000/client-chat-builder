  ...
  return (
    <div
      className={`flex items-center overflow-hidden ${isCollapsed ? 'justify-center p-2' : 'gap-2 px-3 py-2'} mx-2 my-1 rounded-lg transition-all duration-200 ${
        isDisabled
          ? 'cursor-not-allowed bg-slate-100 dark:bg-white/[0.04] text-slate-400 dark:text-white/30 opacity-60'
          : 'wf-node cursor-grab bg-white dark:bg-white/[0.08] hover:bg-purple-50 dark:hover:bg-purple-900/20 shadow-sm hover:shadow-md border border-slate-200 dark:border-white/[0.12] hover:border-purple-300 dark:hover:border-violet-500/40 active:scale-95'
      }`}
    ...
    >
      ...
      {!isCollapsed && <span className="font-medium text-xs text-slate-700 dark:text-white/80 leading-tight min-w-0 break-words">{label}</span>}
    </div>
  );
};
...
export const AgentComponentSidebar = ({ agent, isCollapsed = false, onToggle }: AgentComponentSidebarProps) => {
  ...
  return (
    <aside className={`${isCollapsed ? 'w-0 overflow-hidden md:w-12' : 'overflow-y-auto fixed md:relative inset-y-0 left-0 z-50 md:inset-auto w-[280px] md:w-[232px] shadow-2xl md:shadow-none'} bg-gradient-to-b from-slate-50 to-white dark:from-background dark:to-background ${isRTL ? 'border-l' : 'border-r'} border-slate-200 dark:border-white/[0.08] transition-all duration-300 ease-in-out scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-white/10`}>
      {/* Header with toggle button */}
      <div className={`sticky top-0 z-10 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} p-4 bg-gradient-to-b from-slate-50 via-slate-50 to-transparent dark:from-background dark:via-background`}>
        {!isCollapsed && (
          <div>
            <h3 className="text-base font-bold text-slate-800 dark:text-white">
              {t('builder.components')}
            </h3>
            <p className="text-xs text-slate-500 dark:text-white/45">Drag to canvas</p>
          </div>
        )}
        {onToggle && (
          <Button
            ...
            size="icon"
            onClick={onToggle}
            className="h-8 w-8 flex-shrink-0 hover:bg-slate-200 dark:hover:bg-white/[0.08] rounded-lg"
      ...
      {isCollapsed ? (
        ...
        <AccordionItem value="item-1" className="border border-slate-200 dark:border-white/[0.10] rounded-xl overflow-hidden bg-white dark:bg-white/[0.04]">
          <AccordionTrigger className="hover:no-underline px-4 py-3 text-sm font-semibold text-slate-700 dark:text-white/80 hover:bg-slate-50 dark:hover:bg-white/[0.06]">{t('builder.tools')}</AccordionTrigger>
          <AccordionContent>
            {isLoadingTools ? (
              <div className="flex items-center justify-center py-4">
                <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-green-600"></div>
        ...
        <AccordionItem value="item-2" className="border border-slate-200 dark:border-white/[0.10] rounded-xl overflow-hidden bg-white dark:bg-white/[0.04]">
          <AccordionTrigger className="hover:no-underline px-4 py-3 text-sm font-semibold text-slate-700 dark:text-white/80 hover:bg-slate-50 dark:hover:bg-white/[0.06]">{t('builder.knowledgeBases')}</AccordionTrigger>
          <AccordionContent>
            {isLoadingKnowledgeBases ? (
              <div className="flex items-center justify-center py-4">
                <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-green-600"></div>
        ...
        <AccordionItem value="item-3" className="border border-slate-200 dark:border-white/[0.10] rounded-xl overflow-hidden bg-white dark:bg-white/[0.04]">
          <AccordionTrigger className="hover:no-underline px-4 py-3 text-sm font-semibold text-slate-700 dark:text-white/80 hover:bg-slate-50 dark:hover:bg-white/[0.06]">
