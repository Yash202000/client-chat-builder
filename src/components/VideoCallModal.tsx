...
export const VideoCallModal: React.FC<VideoCallModalProps> = ({ sessionId, userId, onClose, preloadedToken, livekitServerUrl, conversationSessionId, conversationAgentId }) => {
  ...
  const { data: messages = [] } = useQuery<ConvMessage[]>({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`${API_BASE_URL}/api/v1/conversations/sessions/${encodeURIComponent(conversationSessionId!)}/messages?limit=50`, {
  ...
  // Real-time conversation WebSocket with auto-reconnect
  useEffect(() => {
    if (!canChat) return;
  ...
  // Track scroll position to show/hide the jump button
  useEffect(() => {
    const el = messagesContainerRef.current;
    if (!el) return;
    const handleScroll = () => {
  ...
  // On new messages: auto-scroll if already at bottom, otherwise show badge
  useEffect(() => {
  ...
  useEffect(() => {
    if (preloadedToken) return; // skip fetch — token already provided
    (async () => {
      try {
      ...
      >
        ...
        <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-indigo-500 to-blue-500 p-[2px]">
          <div className="w-full h-full rounded-2xl bg-white dark:bg-slate-900 overflow-hidden">
            ...
            <div className="w-full h-full lk-hide-chat-toggle lk-mini-controls">
              <LiveKitRoom
                video={true}
                audio={true}
                token={token}
  ...
  const innerContent = (
    <div className={`w-full h-full flex flex-col overflow-hidden ${isMaximized ? '' : `rounded-2xl`} bg-white dark:bg-slate-900 ${theme}`}>
            ...
            <div className="flex-1 overflow-hidden flex">
              {/* LiveKit video — always hide built-in chat toggle, we have our own */}
              <div className="flex-1 overflow-hidden bg-gray-100 dark:bg-slate-950 lk-hide-chat-toggle">
                <LiveKitRoom
                  video={true}
                  audio={true}
                  token={token}
              ...
              {/* Real chat sidebar — always open when toggled */}
              {isChatOpen && (
                          ...
                          return (
                            <div key={msg.id} className={cn('flex w-full mb-2', isAgent ? 'justify-end' : 'justify-start')}>
                              <div className={cn(
                                ...
                                {!isAgent && (
                                  <p className="text-[10px] font-semibold opacity-60 mb-0.5">Customer</p>
                                )}
  ...
  return createPortal(
    isMaximized ? (
      <div className={`fixed inset-0 z-[99999] ${theme}`}>
        {innerContent}
      </div>
    ) : (
      <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-[99999]">
        ...
        <div
          className="relative w-[95%] h-[95%] max-w-[1600px] rounded-2xl overflow-hidden"
