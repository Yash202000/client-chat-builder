          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center flex-shrink-0 shadow-md">
                {tool.category?.icon ? <i className={`${tool.category.icon} text-base text-white`}></i> : <Wrench className="h-5 w-5 text-white" />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h1 className="text-lg font-bold text-slate-900 dark:text-white truncate">{tool.name}</h1>
                  {tool.category && (
                    <Badge variant="secondary" className="bg-violet-50 dark:bg-violet-500/10 text-violet-700 dark:text-violet-300 border border-violet-200 dark:border-violet-700/50 flex-shrink-0">
                      {tool.category.name}
                    </Badge>
                  )}
                </div>
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={handleFavorite} className="h-8 text-xs border-slate-200 dark:border-slate-700 hover:bg-yellow-50 dark:hover:bg-yellow-900/20">
                <Star size={13} className={`${tool.is_favorited ? 'fill-yellow-400 text-yellow-400' : ''} mr-1`} />
                {tool.is_favorited ? 'Favorited' : 'Favorite'}
              </Button>
              <Permission permission="ai-tool:update">
                <Button variant="outline" size="sm" onClick={() => navigate(`/dashboard/ai-tools/${id}/edit`)} className="h-8 text-xs border-slate-200 dark:border-slate-700 hover:bg-violet-50 dark:hover:bg-violet-900/20">
                  <Edit size={13} className="mr-1" />Edit
                </Button>
              </Permission>
              <Permission permission="ai-tool:delete">
                <Button variant="outline" size="sm" onClick={handleDelete} className="h-8 text-xs text-red-600 dark:text-red-400 border-red-200 dark:border-red-800/50 hover:bg-red-50 dark:hover:bg-red-900/20">
