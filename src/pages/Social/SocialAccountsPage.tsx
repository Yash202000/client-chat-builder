...
interface SocialAccount {
  id: number;
  platform: 'linkedin' | 'instagram' | 'facebook' | 'reddit' | 'twitter';
  account_name: string;
  account_type: string;
...
const PLATFORMS = {
  ...
  reddit: {
    label: 'Reddit', Icon: RedditIcon, channel: 'CH·04', tagline: 'Community reach',
    accent: '#FF4500', gradient: 'linear-gradient(135deg,#FF4500,#FF6534)', glow: '255,69,0',
    scopes: ['identity', 'submit', 'read'],
  },
  twitter: {
    label: 'X (Twitter)', Icon: TwitterXIcon, channel: 'CH·05', tagline: 'Real-time reach',
    accent: '#000000', gradient: 'linear-gradient(135deg,#000000,#333333)', glow: '0,0,0',
...
export default function SocialAccountsPage() {
  ...
  const PLATFORM_TAGLINES: Record<string, string> = {
    ...
    reddit: t('social.accounts.redditTagline', 'Community reach'),
    twitter: t('social.accounts.twitterTagline', 'Real-time reach'),
  };
  ...
  const handleConnect = (platform: string) => {
    ...
    const token = localStorage.getItem('accessToken');
    const url = `/api/v1/social/auth/${platform}/connect?token=${token}`;

    const popup = window.open(url, `${platform}_oauth`, 'width=600,height=700,scrollbars=yes,resizable=yes');

    const onMessage = (event: MessageEvent) => {
      if (event.origin !== window.location.origin) return;
      if (event.data?.type === 'oauth_complete' || event.data?.type === 'oauth_error') {
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 shrink-0 bg-card border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] font-mono text-muted-foreground">
              {isLoading ? '—' : `${totalActive} / 5 active`}
            </span>
          </div>
