    ...
    <div className="p-6" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-muted text-muted-foreground border border-border">
            ...
            {tools?.length ?? 0} total
          </span>
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
            <Code className="h-3 w-3" />
            {customCount} {t("tools.toolTypes.custom")}
          </span>
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border border-cyan-500/20">
            ...
            {mcpCount} {t("tools.toolTypes.mcpConnection")}
          </span>
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-violet-500/10 text-violet-600 dark:text-violet-400 border border-violet-500/20">
            ...
            {builtinCount} {t("tools.toolTypes.builtin")}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            ...
            className="h-8 px-3 text-xs border-violet-500/30 text-violet-600 dark:text-violet-400 hover:bg-violet-500/10"
          >
      ...
      {isLoadingTools ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-sm font-semibold text-foreground mb-1">{t("tools.noToolsFound")}</h3>
          <p className="text-xs text-muted-foreground max-w-xs mb-5">{t("tools.createFirstTool")}</p>
            ...
            {filteredTools.map((tool, i) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {tool.tool_type === 'mcp' && tool.mcp_server_url && (
                          <p className="text-[10px] font-mono text-muted-foreground mt-1 truncate">{tool.mcp_server_url}</p>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Card footer */}
                  <div className={cn("px-4 py-2.5 border-t border-border bg-muted/30 flex items-center gap-1.5", isRTL && "flex-row-reverse")}>
                    {(tool.tool_type !== 'builtin' || isSuperAdmin) && (
                      <Dialog open={isEditDialogOpen && selectedTool?.id === tool.id} onOpenChange={(isOpen) => { if (!isOpen) setSelectedTool(null); setIsEditDialogOpen(isOpen); }}>
                        <DialogTrigger asChild>
                          <Button variant="outline" size="sm" onClick={() => setSelectedTool(tool)} className="h-7 px-2.5 text-xs">
                    ...
                    {tool.tool_type === 'custom' && (
                      <Button
                        ...
                        className="h-7 px-2.5 text-xs border-violet-500/30 text-violet-600 dark:text-violet-400 hover:bg-violet-500/10"
                      >
                    ...
                    {tool.tool_type === 'custom' && (
                      <Dialog open={isTestDialogOpen && selectedTool?.id === tool.id} onOpenChange={(isOpen) => { if (!isOpen) setSelectedTool(null); setIsTestDialogOpen(isOpen); }}>
                        <DialogTrigger asChild>
                          <Button variant="outline" size="sm" onClick={() => setSelectedTool(tool)} className="h-7 px-2.5 text-xs border-blue-500/30 text-blue-600 dark:text-blue-400 hover:bg-blue-500/10">
                    ...
                    {tool.tool_type !== 'builtin' && (
                      <button
                        ...
                        className="h-7 w-7 rounded flex items-center justify-center text-muted-foreground hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors ml-auto"
